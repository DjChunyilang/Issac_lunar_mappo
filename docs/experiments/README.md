...
| exp024 | 随机增强 lunar crater proxy | shared-joint MAPPO pure RL + mutual path safety subgoal filter | seed23 20M running | 运行中 | 新增候选路径与可见邻居 raw path 的同时间采样比较，补 exp023 动态路径盲点；不改变 86/54 接口。 | [exp_024_randomized_terrain_mutual_path_filter.md](exp_024_randomized_terrain_mutual_path_filter.md) |
