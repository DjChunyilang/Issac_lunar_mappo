...
| exp023 | 随机增强 lunar crater proxy | shared-joint MAPPO pure RL + soft progress-preserving subgoal filter | seed23 20M running | 运行中 | 针对 exp022 安全 standoff：降低 filter 介入和安全权重，保留 center-progress 软约束，只在 raw collision 时 override；使用 `progress_preserving_long` 选 checkpoint。 | [exp_023_randomized_terrain_soft_progress_filter.md](exp_023_randomized_terrain_soft_progress_filter.md) |
