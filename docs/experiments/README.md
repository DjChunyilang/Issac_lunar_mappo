...
| exp021 | 随机增强 lunar crater proxy | shared-joint MAPPO pure RL + 课程化 terrain/safety 子目标过滤 | seed23 20M 运行中 | 待评估 | 针对 exp020 hard filter 过强问题，前 2048 timesteps 不替换 action，随后把 filter 介入概率和 score scale 课程化提高，并加入 raw-risk/deviation 辅助惩罚；工程测试和 smoke 已通过。 | [exp_021_randomized_terrain_filter_curriculum.md](exp_021_randomized_terrain_filter_curriculum.md) |
