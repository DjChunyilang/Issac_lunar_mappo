...
| exp022 | 随机增强 lunar crater proxy | shared-joint MAPPO pure RL + endpoint/path safety constrained curriculum filter | seed23 20M planned/running | 待评估 | 基于 exp021 的高 collision 失败模式，加入 endpoint/path safety hard constraint、center-progress constraint 和 warmup 后 safety override；目标是在保留集合趋势的同时降低 collision。 | [exp_022_randomized_terrain_endpoint_safety_filter.md](exp_022_randomized_terrain_endpoint_safety_filter.md) |
