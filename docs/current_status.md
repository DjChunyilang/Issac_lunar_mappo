...
- exp024 已开始 mutual path safety filter 迭代：在 exp023 基础上把可见邻居 raw subgoal path 作为动态障碍，按相同时间采样比较候选路径；完整 pytest、CPU smoke、CUDA smoke 已通过，seed23 20M 长训已作为 `exp024-mutual-path-filter-20m.service` 启动。
...
| exp024 | 随机增强 lunar crater proxy | shared-joint MAPPO pure RL + mutual path safety subgoal filter | 运行中 | 在 exp023 基础上新增 mutual path safety：候选路径与可见邻居 raw path 同时间采样比较；不改变 86/54 接口，不恢复 hard safety constraint。 |
...
- exp024 在训练完成前只能表述为“mutual path safety filter 修复假设/工程验证”，不能写成随机地形安全策略改善或收敛。
...
1. 当前进行 exp024：补上 exp023 static filter 的动态路径盲点，验证 mutual path safety 是否能降低 late-stage collision，同时保留 exp023 恢复的集合进度。
...
3. 如果 exp024 仍失败，下一步应转向 action representation / planner projection / success geometry，而不是继续加大 post-processing filter 权重。
