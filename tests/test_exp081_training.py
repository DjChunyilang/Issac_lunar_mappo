...
CONFIG = (
    ...
    / "configs"
    / "experiment"
    / "exp081_structured_bicycle_quintic_map25_terminal_slot_capture.yaml"
)


def _without_exp081_capture_delta(raw: dict) -> dict:
...
def test_exp081_isolates_per_rover_terminal_slot_capture() -> None:
    baseline = load_yaml(BASELINE)
    exp081 = load_yaml(CONFIG)
    assert _without_exp081_capture_delta(exp081) == _without_exp081_capture_delta(
        baseline
    )

    control = cfg_from_experiment(CONFIG).low_level_control
    assert control.terminal_slot_capture_enabled
    assert control.terminal_slot_capture_blend == pytest.approx(0.65)
